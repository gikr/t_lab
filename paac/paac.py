    ...
    def train(self):
        ...
        while self.global_step < max_global_steps:
            ...
            for t in range(max_local_steps):
                ...
                if flg == True:
                    self.length_previous = np.asarray(self.length_previous) + np.asarray([5, 5])
                    len_int = list(self.length_previous)
                    self.batch_env.set_difficulty(len_int)

                #if self.check_progress(dones, rs) == True:
                #    self.batch_env.set_difficulty()
